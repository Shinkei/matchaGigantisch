...
class BlogPostDAO:
    ...
    def get_posts(self, num_posts):
        ...
        cursor = self.posts.find().sort('_id', pymongo.DESCENDING).limit(num_posts)
    ...
    def add_comment(self, permalink, name, email, body):
        ...
        try:
            ...
            query = {'permalink':permalink} #query to get the post
            post = self.posts.find_one(query)
            post['comments'].append(comment) #add the comment to the post

            self.posts.update({'_id':post['_id']},post,safe=True) #update the post in mongo
