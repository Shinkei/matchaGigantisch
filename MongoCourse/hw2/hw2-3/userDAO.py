...
class UserDAO:
    ...
    def validate_login(self, username, password):
        ...
        try:
            ...
            passwords = self.users.find({"_id":username})
            for passwordTem in passwords:
                salt = passwordTem["password"].split(",")[1]
                passwordGenerado = self.make_pw_hash(password, salt)
                query = {"_id":username,"password":passwordGenerado}
